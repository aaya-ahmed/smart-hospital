...
@Component({
  selector: 'app-indoor-records',
  templateUrl: './indoor-records.component.html',
  styleUrls: ['../../styles/datatable.css','./indoor-records.component.css']
})
export class IndoorRecordsComponent implements OnInit {
  record:any;
  patientReport:any
  patientLabIds:any[]=[];
  prescriptions:any[]=[];
  ...
  getreportInfo(index:number){
    ...
    this.service.get('Patient/GetPatientReport/'+this.patient.id+'/'+this.record.discahrgeDate).subscribe(
    res=>{
  ...
  pdf.setFontSize(13)
  pdf.text('Phone Number : ',100,60);
  pdf.setFontSize(12)
  pdf.text(this.patient.phoneNumber,135,60);
  pdf.text('Exit Date : ',100,70);
  pdf.setFontSize(12)
...
if(this.patientReport.result.listOfMedicineNames.length>0){
  ...
  pdf.setFontSize(12)
  pdf.text(this.patient.firstName+' '+this.patient.lastName,40,40);
  pdf.setFontSize(13)
 ...
 // prescription Items
   ...
   console.log(this.prescriptionInfo.prescription.prescriptionItems)
